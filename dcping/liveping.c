...
static int32_t create_icmp_socket(char *remote, char *local, int16_t family,
        int32_t protocol, const char *iface, int32_t mega_scale)
{
    ...
    */
    ...
    rcvbufsize = mega_scale * 1024 * 1024;
    sndbufsize = mega_scale * 1024 * 1024;
...
void * do_work(void *data) {
    ...
    raw_fd = create_icmp_socket(NULL, g_local, g_family, g_protocol, g_iface, targ->mega_scale);
    if (raw_fd == -1) {
        exit(EXIT_FAILURE);
    }
...
static void usage()
{
    ...
    printf("\t-d : duration between repeat\n");
...
int main(int argc, char *argv[])
{
    ...
    int i = 0;
    char *ofile_prefix = "";
    int32_t repeats = 100, sleep_duration = 100;
    int mega_scale = 128;

    while ((opt = getopt(argc, argv, "m:i:c:d:r:n:f:t:o:p:l:h6")) != -1) {
        switch (opt) {
        case '6':
            g_family = AF_INET6;
            g_protocol = 0x3A;
        ...
        case 'd':
            ...
            if (sleep_duration < 1 || sleep_duration > 10000) {
                fprintf(stderr, "-d is invalid (0 < t <= 10000)\n");
                exit(EXIT_FAILURE);
            }
            break;
        case 'r':
    ...
    fprintf(stdout, "Bytedance Tookit %s @Bytedance SYS ver 20220222\n", argv[0]);
    fprintf(stdout, "Interface: %s Threads: %d Hosts: %d Buffer_MB: %d\n", g_iface, n_workers, nLine, mega_scale);
    ...
    while(nloop < repeats) {
        ...
        usleep(sleep_duration * 100);
        fprintf(stdout, " *** %03d: total %ld send\n",
                nloop, n_ping_send);
    }

    sprintf(tmpbuf, "/tmp/%sping_res_OK.txt", ofile_prefix);
    FILE *fp1 = fopen(tmpbuf, "w");
    sprintf(tmpbuf, "/tmp/%sping_res_Fail.txt", ofile_prefix);
    FILE *fp2 = fopen(tmpbuf, "w");
    int len = 0;
    int seq_ok = 0;
    int seq_fail = 0;
    for(i = 0; i < nLine; i++) {
        struct record *e = iplist[i];
        if(e->latency == -1) {
            len = snprintf(tmpbuf, sizeof(tmpbuf), "%s %d %d #%d\n",
                    e->name, e->seq, e->latency, ++seq_fail);
            fwrite(tmpbuf, len, 1, fp2);
        } else {
            len = snprintf(tmpbuf, sizeof(tmpbuf), "%s %d %d #%d\n",
                    e->name, e->seq, e->latency, ++seq_ok);
