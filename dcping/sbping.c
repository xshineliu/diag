...
int main(int argc, char *argv[])
{
    ...
    int i = 0;
    char *ofile = "/dev/shm/ping_lostconn_5min.txt";

    while ((opt = getopt(argc, argv, "i:c:n:f:o:s:p:l:h6")) != -1) {
        switch (opt) {
        case '6':
            g_family = AF_INET6;
            g_protocol = 0x3A;
	...
	fprintf(stdout, "Bytedance Tookit %s @Bytedance SYS ver 20200316\n", argv[0]);
    ...
    while(1) {
    	...
    	for(i = 0; i < nLine; i++) {
			...
			if(e->lost > 29) {
    			nlost3++;
    			if(dump && fp) {
					int len = snprintf(tmpbuf, sizeof(tmpbuf), "%s %ld %ld #%d\n", e->name,
							(ts - e->lost_start) / 1000, e->lost_start / 1000, ++idx_failed);
					fwrite(tmpbuf, len, 1, fp);
    			}
			}
			if(e->lost > 89) {
    	...
    	if(dump) {
    		...
    		if(fp) {
    			int len = snprintf(tmpbuf, sizeof(tmpbuf), "### dump end total %d lost connection\n", idx_failed);
    			fwrite(tmpbuf, len, 1, fp);
    			fclose(fp);
    			fp = NULL;
    		}
